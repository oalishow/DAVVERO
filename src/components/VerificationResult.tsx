  ...
  return (
    <div className="w-full mt-2 print:mt-1 animated-fade-in flex flex-col items-center">
      ...
      >
        ...
        <h2 className={`text-base sm:text-xl font-black mb-0.5 sm:mb-1 uppercase tracking-widest ${status === 'VALID' ? 'text-emerald-600 dark:text-emerald-400' : status === 'INACTIVE' ? 'text-amber-600 dark:text-amber-400' : 'text-rose-600 dark:text-rose-400'}`}>
          {titleText}
        </h2>
        <p className={`text-[10px] sm:text-xs font-semibold uppercase tracking-widest mb-3 sm:mb-5 ${status === 'VALID' ? 'text-emerald-500' : status === 'INACTIVE' ? 'text-amber-500' : 'text-rose-500'}`}>
          {subtitleText}
        </p>
        ...
        <div className="space-y-2 sm:space-y-3 bg-slate-50 dark:bg-slate-800/50 p-3 sm:p-4 rounded-2xl border border-slate-200 dark:border-slate-700/50 text-left">
          <div className="text-center">
              <p className="text-[9px] sm:text-[10px] text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-0.5 sm:mb-1 font-semibold">{status === 'NOT_FOUND' ? 'Tentativa de Acesso' : 'Nome Completo'}</p>
              <p className="text-base sm:text-xl font-bold text-slate-800 dark:text-white leading-tight">{safeName}</p>
              {member?.ra && <p className="text-[9px] font-medium text-slate-500 mt-0.5">RA: {member.ra}</p>}
              
              {member?.roles && member.roles.length > 0 && (
                <div className="mt-1 flex flex-wrap justify-center gap-1">
                  {member.roles.map(r => <span key={r} className="px-1.5 py-0.5 bg-sky-100 dark:bg-sky-500/20 text-sky-700 dark:text-sky-300 rounded text-[8px] uppercase border border-sky-200">{r}</span>)}
                </div>
              )}
              {member?.course && (
                <div className="mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-1.5 text-center">
                  <p className="text-[8px] text-slate-500 uppercase tracking-widest font-semibold">Curso Matriculado</p>
                  <p className="text-[10px] sm:text-xs font-bold text-sky-600 dark:text-sky-400 uppercase mt-0.5">{member.course}</p>
                </div>
              )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-200 dark:border-slate-700/50">
              <div>
                  <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase tracking-widest mb-0.5 font-semibold">Status</p>
                  <p className={`text-[10px] sm:text-xs font-bold flex items-center gap-1 ${status === 'VALID' ? 'text-emerald-600' : status === 'INACTIVE' ? 'text-amber-600' : 'text-rose-600'}`}>
                      <span className={`w-1.5 h-1.5 rounded-full ${dotColor}`}></span> {badgeText}
                  </p>
                  {status !== 'NOT_FOUND' && <p className="text-[9px] text-slate-500 mt-0.5">{status === 'EXPIRED' ? 'Venceu a:' : 'Vence a:'} <span className="text-slate-700 dark:text-slate-300 font-medium">{safeDate}</span></p>}
              </div>
              <div>
                  <p className="text-[9px] sm:text-[10px] text-slate-500 uppercase tracking-widest mb-0.5 font-semibold">Cód. Uso</p>
                  <p className="text-[10px] font-mono text-slate-700 dark:text-slate-200 flex flex-col">
                      <span>{safeCode}</span>
                      {member?.diocese && <span className="text-[9px] text-sky-600 dark:text-sky-400 mt-1 font-sans font-bold">DIOCESE: {member.diocese}</span>}
                      {member && <span className="text-[7px] text-slate-400 mt-1 font-sans font-semibold">FAJOPA</span>}
                  </p>
              </div>
          </div>
        </div>

        <div className={`mt-3 p-2 rounded-xl border text-left ${
          ...
          <p className="text-[9px] uppercase tracking-widest mb-0.5 font-semibold opacity-70">Detalhes</p>
        ...
        <div className="mt-3 inline-flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-700">
           <span className="text-slate-600 dark:text-slate-400 text-[9px] font-mono">Processado em: <strong className="text-slate-800 dark:text-slate-200">{timestampStr}</strong></span>
        </div>
        ...
        {member?.alphaCode && (
          <div className="mt-3 flex flex-col items-center gap-1 bg-white p-2 rounded-xl w-fit mx-auto border-2 border-slate-200 shadow-sm">
            <QRCodeSVG 
              ...
              includeMargin={true}
            />
            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{member.alphaCode}</span>
          </div>
        )}
      </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-2 mt-2 w-full max-w-sm no-print print:hidden">
        ...
        <div className="flex gap-2 flex-1">
          <button onClick={handlePrint} className="p-2.5 rounded-xl bg-slate-800 text-white hover:bg-slate-700 transition-colors" title="Imprimir">
            <Printer className="w-4 h-4 sm:w-5 sm:h-5" />
          </button>
          <button onClick={handleExport} disabled={exporting} className={`flex-1 flex justify-center items-center py-2.5 px-4 rounded-xl text-xs sm:text-sm font-bold text-white transition-colors ${
            ...
            {exporting ? '...' : (isMyID && status === 'VALID' ? 'Baixar PDF' : 'Baixar Imagem')}
          </button>
        </div>
      </div>
